...
Перед инсталляцией необходимо применить патч для PostgreSQL. В репозитории находятся два варианта патча: 
1. after_24225ad9aaf.patch - для версий PostgreSQL начиная с коммита 24225ad9aaf (включительно).
2. before_24225ad9aaf.patch - для версий до коммита 24225ad9aaf.
...
При каждом вызове команды EXPLAIN с параметром get_paths в таблицу ee.paths будут записываться все пути, которые оптимизатор запросов рассматривал при поиске лучшего плана исполнения SQL запроса. Помимо этого в таблицу ee.query будет записана общая инфомрация об исполннеии команды EXPLAIN. Если версия PostgreSQL не поддерживает EXPLAIN-параметры (17 версия и младше), то для активации режима заполнения таблиц ee.query и ee.paths необходимо ee.get_paths
...
Информацию из таблицы ee.paths можно по-разному интерпретировать. Примером может послужить утилита [ee_visualizer](https://github.com/04ina/ee_visualizer), визуализирующая пути в удобном формате.
